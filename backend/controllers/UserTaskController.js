import db from "../config/Database.js";
import express from "express";
import cors from "cors";
import moment from 'moment'

export const getuserTask = async(req, res)=>{
    const sql = "SELECT t.*, u.username, u.id as uid, p.id AS pid, p.project_name FROM tasks AS t, users AS u, projects AS p WHERE t.assign_user_id= u.id AND t.project_id = p.id AND u.id = ?";

  const id = req.params.id;
  db.query(sql,[id], (err, data) => {
        if (err) {
        console.log(err);
        return res.json(err);
        }
        return res.json(data);
    });
}

export const saveuserTasks = async(req, res)=>{

  console.log('ok');
  const sql = "INSERT INTO tasks(`task_name`, `assign_user_id`, `project_id`,status,`task_description`, `note`, `asssign_date`, `completed_date`) VALUES (?)";
  const values = [
    req.body.taskName,
    req.body.taskPerson,
    req.body.projectName,
    req.body.taskStatus,
    req.body.taskNote,
    req.body.taskDesc,
    moment(req.body.asssignDate).utcOffset('+0600').format('yyyy-MM-DD'),
    moment(req.body.completeDate).utcOffset('+0600').format('yyyy-MM-DD'),
  ];
//console.log(values)

  db.query(sql, [values], (err, data) => {
    if (err) return res.send(err);
    return res.json(data);
  });
}

export const getuserTaskById = async(req, res)=>{
  const sql = "SELECT t.*, u.username, u.id as uid, p.id AS pid, p.project_name FROM tasks AS t, users AS u, projects AS p WHERE t.assign_user_id= u.id AND t.project_id = p.id AND t.id  = ? ";
  const id = req.params.id;
  db.query(sql,[id], (err, data) => {
    if (err) {
      console.log(err);
      return res.json(err);
    }
    return res.json(data);
  });
}



export const updateuserTask = async (req, res) => {

  const sql = "UPDATE tasks SET `task_name`= ?, `assign_user_id`= ? , `project_id`= ?,`status`= ?,`note`= ?, `task_description`=?, `asssign_date`= ?, `completed_date`= ? WHERE id = ?";
  const id = req.params.id;
  const values = [
    req.body.taskName,
    req.body.taskPerson,
    req.body.projectName,
    req.body.taskStatus,
    req.body.taskNote,
    req.body.taskDesc,
    moment(req.body.asssignDate).utcOffset('+0600').format('yyyy-MM-DD'),
    moment(req.body.completeDate).utcOffset('+0600').format('yyyy-MM-DD'),
    id,
  ];

    db.query(sql, [...values], (err, result) => {
      if (err) return res.json(err);
      return res.json(result);
    });
}

// export const deleteuserTask = async (req, res) => {
//   console.log('ok')
//   // const taskId = req.params.id;
//   // const q = " DELETE FROM tasks WHERE id = ? ";

//   // db.query(q, [taskId], (err, data) => {
//   //   if (err) return res.send(err);
//   //   return res.json(data);
//   // });
// }


